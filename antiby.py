...
class Eat:
	...
	r = 0
	c = 0
	def __init__(self, x,y,r,c,t_s=0,E=3):
		self.x = x
		self.y = y
		self.r = r
		self.c = c
		self.E = E
		self.H = (x,y,r,c,E,t_s)
...
def decide(eat, bac, T):
	...
	if len(eat)!=0 and len(bac)!=0:
		for k in range(len(bac)):
			...
			if w[2]<=wmin:
				if T:
					dE = eat[sort(bac[k],eat)].H[4]
					dt = eat[sort(bac[k],eat)].H[5]
				else:
					dE = eat[sort(bac[k],eat)].H[6]
					dt = 0
...
def define(t):
	...
	if (t%tN)==0:
		if t<tJ:
			...
			for k in range(int(N*math.tan(sT*p/200))):
				x = random.randint(0,W)
				y = random.randint(0,W)
				if 100*random.random()<sT:
					EAT.append(Eat(x,y,p,ant_color, eff))
				else:
					EAT.append(Eat(x,y,p,eat_color))
